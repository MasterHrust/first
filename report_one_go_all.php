...
$sql = mysqli_query($link,"SELECT ogne.number,DATE_FORMAT(kvart.date, '%d.%m.%Y') AS date,kvart.vid,kvart.vn_vid,kvart.weight,kvart.power,kvart.sost,kvart.meri,kvart.fio from kvart LEFT JOIN ogne on ogne.id=kvart.ogne_id WHERE kvart.ogne_id= *");
$sql2 = mysqli_query($link,"SELECT number, DATE_FORMAT(date, '%d.%m.%Y') AS date, place, type, izg, number_zav, date_izg, model FROM ogne WHERE id = *");
